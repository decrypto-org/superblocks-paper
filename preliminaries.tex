...
organized into a compact data structure such as a Merkle tree~\cite{merkle}
\TODO{TODO citation} containing one leaf per superblock level $\mu$. The number
...
transaction is implemented using an \textsf{OP\_RETURN} committing to the Merkle
tree root containing the interlink list in its leafs. User-activated velvet
forks allow the adoption of a new rule without requiring miners to upgrade their
...
\TODO{TODO cite christoglou}. Improving their size has direct financial impact
