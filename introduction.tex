...
\begin{enumerate}
  ...
  \item We describe the simple but important optimization in regards to the way
        ...
        independently by the Nimiq blockchain~\cite{nimiq-dev-ref}, but they do
        not organize their interlink into a Merkle tree. Thus, their
        construction requires sending the complete list of $\Theta(\lg(|C|))$
        superblocks rather than a $\Theta(\lg\lg(|C|))$ inclusion proof for each
        block, yielding worse proof sizes than the present work.}
