...
We will now show that the savings from the interlink set technique are significant, specifically that $\EX[B^n] < \frac{1}{2}\lg(n) + \Omega(1)$.
...
\end{lemma}
\begin{proof}
  By induction on $k$. Clearly it holds for $k=0$:
...
We obtain that $\EX[B^n] < \frac{1}{2}(\lg(n)+2+e)$. Therefore the expected savings are $\lg(n) - \EX[B^n] > \frac{1}{2}(\lg(n)-2-e)$. Solving for $n$ we obtain that $n > 26$ is required for savings to exist.
